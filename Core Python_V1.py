...
=================
  I. Introduction
        1. Python Features. Explain in detail 
        2. Python Advantages, Disadvantages
 II. VARIABLES
III. OPERATORS
...
VIII.Packages, Modules
  IX.Exception Hadling
   X.File handling
  XI.Iterator,Generator
 XII.Decorator
XIII.Python 2.x vs 3.x
XIV. Regular Expressions
 XV. Networking
XVI. Datetime:
XVII.Database interaction:

I. INTRODUCTION
*****************
1. Python Features. Explain in detail 
	 
	 1.easy to learn : Add description about each one
     2.easy to read
...
---------------------------------------------------------------------------------------------------------------------
3. Compile (OR) Compilation Time vs Runtime 
...
------------------------------------------------------------------------------------------------------------------
4. Why Python is called as Interprted Programming Language and not compiled ?
     - Python is called intrepretted langauge because it goes through an intrepreter ,
       which turns the code you write into the language understood by computers proccessor
---------------------------------------------------------------------------------------------------------------------
5. Tokens in Python 
...
1. VARIABLE SCOPES OF LEGB RULE
    - The LEGB rule names the possible scopes of a variable in Python: Local, Enclosing, Global, Built-in.
      ...
      Local    : <Add Notes here>
      Enclosed : <Add Notes here>
      Global   : <Add Notes here>
      Builtin  : <Add Notes here>
...
2. MEMORY ALLOCATION OF VARIABLES
    - Memory allocation can be defined as allocating a block of space in the computermemory to a program.
       ...
       collection for python so that the user does not have to do manual garbage collection
        x = 10
        Write detailed steps here 
        print(x)
        Write detaild steps here
...
3. / // % DIFFERENCES
    ...
    '//' Floor Division	--> The division of operands where the result is the quotient in which the digits 
                            ...
                            But if one of the operands is negative, the result is floored, i.e., rounded away
...
2. WHILE VS FOR
       ...
       While loop – This loop statement checks for a condition at the beginning and till the condition is 
                    fulfilled, it will execute the body of the loop.

       For loop   – For loops are used to sequentially iterate over a python sequence.
                    When the sequence has been iterated completely, the for loop ends and thus executes the 
                    next piece of code.
...
13. Collections module classes, properties
  ...
  - Frozen set 
      The frozenset() function returns an immutable frozenset object initialized with elements from the given 
      iterable.Elements of the frozen set remain the same after creation.
...
14.Lambda function. Examples with map filter reduce functions
   ...
   --A lambda function is a small anonymous function. A lambda function can take any number of arguments, but can only have one expression.
     ...
     map()   : Applies a function on all the items of an iterator given as input. 
               An iterator, for example, can be a list, a tuple, a set, a dictionary, a string, and 
               it returns an iterable map object. 
    ...
    reduce() : is a function that implements a mathematical technique called folding or reduction. 
                reduce() is useful when you need to apply a function to an iterable and reduce it to a 
...
15.range vs xrange
     ...
     The xrange() function in Python is used to generate a sequence of numbers, 
      ...
      In Python 2.x we have 2 functions range (uses Iterator mechanism), xrange (uses Generator mechanism)
      In Python 3.x we have only range function (which uses Generator mechanism internally)
...
2. What are OOPS features
	...
	-classes and objects : < Add Notes here >
	-Encapsulation : < Add Notes here >
	-Abstraction   : < Add Notes here >
	-Inheritance   : < Add Notes here >
	-Polymorphism  : < Add Notes here >
...
3. class vs object. Explain in detail
    ...
    - Class is logical structure, Object is physical structure
    - In class, fields are logical and methods are physical, whereas 
      In Object, fields are physical and methods are logical  
...
4. Explain each feature in detail with examples
   ...
   1. Encapsulation :
        ...
        A class is an example of encapsulation as 
                it binds all the data members (instance variables) and methods into a single unit.
        Object is also an example for encapsulation as
                fields are initialized inside object and methods can be accessed logically 
...
5. Method overloading. Explain in detail
  ...
  -- Methods in Python can be called with zero, one, or more parameters. 
     This process of calling the same method in different ways like by passing different number of 
        arguments is called method overloading. ...
     Two methods can have the same name in Python, but latest method will hide previous method name.
     
     So in Python Method Overloading can't be achived directly but if a method with parameters having 
     default arguments then that method can be called in 2 or more ways. With this process we can 
     achive Method Overloading indirectly.
...
7. Inheritance types.Explain in detail 
   Types of Inheritance:
     ...
     2. Multiple Inheritances-  When a class is derived from more than one base class it is called multiple 
                                Inheritance.The derived class inherits all the features of the base case.
     3. Multilevel inheritance -The multi-level inheritance includes the involvement of at least two or 
                                more than two classes.One class inherits the features from a parent class and 
                                the newly created sub-class becomes the base class for another new class

     4. Hierarchical inheritance - When we derive or inherit more than one child class from one(same) parent 
                                   class.Then this type of inheritance is called hierarchical inheritance.
     5. Hybrid inheritance -  Hybrid inheritance is a combination of simple, multiple inheritance and 
                              hierarchical inheritance
------------------------------------------------------------------------------------------------------------------------------------------------
8. Abstraction Explain in detail
    Abstraction:--  Abstraction in OOP is a process of hiding the real implementation of the method by only 
                    showing a method signature.In Python, we can achieve abstraction using ABC 
                    (abstraction class) or abstract method. ABC is a class from the abc module in Python
...
9. Abstract class vs interface 
   ...
   Abstract class:
      - A class is called an Abstract class if it contains one or more abstract methods. 
        ...
        Abstract classes may not be instantiated, and its abstract methods must be implemented by its subclasse
   Interface:
...
10.Mulitple inheritance. MRO Method Resolution Order priciple
    ...
    -  The Python Method Resolution Order defines the class search path used by Python to search for the right
       method to use in classes having multi-inheritance

------------------------------------------------------------------------------
11. Diamond Problem
    - The diamond problem occurs when two classes have a common ancestor, and another class has both 
...
13. Constructor Overloading
     ...
     -  Python does not support Constructor overloading; it has no form of function. 
        In Python, Methods are defined solely by their name, and there can be only one method per class 
        with a given name.
        But if init method with any of parameters have default values then we can create object 
        in 2 or more ways so that we can indirectly achive Constructor Overloading

--------------------------------------------------------------------------------------------------------
14. __init__ purpose
 
   --- init is short for initialization. It is a constructor which gets called when you make an instance 
       of the class and it is not necessary.But usually it our practice to write init method for setting 
...
__str__  : The __str__ method in Python represents the class objects as a string – it can be used for classes. 
...
------------------------------------------------------------------
16. Access modifiers. Does exist in python.Explain in detail 

	-The access modifiers in Python are used to modify the default scope of variables. 
	- 3 types of access modifiers:
        Public : The members of a class that are declared public are easily accessible from any part of the 
                 program.All data members and member functions of a class are public by default. 

      Protected: The members of a class that are declared protected are only accessible to a class derived 
...
16. Class vs Instance varaibles
  ...
  Class variables:
    ...
    So if we want to work with sharable data, we can define class method with cls as first parameter
    and we can access class variables 
  ...
  Instance Variables:
	...
	- Declared inside the constructor method of class (the __init__ method). 
      They are tied to the particular object instance of the class, hence the contents of an instance variable
      are completely independent from one object instance to the other.
    - If we have to implement behavior uniquely on each object then we can use instance method with self 
      as first parameter and after creating object we can call instace method so that object will be passed
      as argument to self paramter 
...
18. self importance 
	...
	-The self is used to represent the instance of the class. 
     ...
     While calling instance methods using object, first object will be passed as argument to self parameter 
...
-------------------------------
3. recursive import issue in Python 
    < Add Notes here >
...
json : It is the string version that can be read or written to a file. Python has a JSON module that will 
       help converting the datastructures to JSON strings. 
	   The JSON module is mainly used to convert the python dictionary above into a JSON string that can be 
       written into a file.
gzip : Python's gzip module is the interface to GZip application. The gzip data compression algorithm itself 
       is based on zlib module. The gzip module contains definition of GzipFile class along with its methods. 
...
xwlt : This is a library for developers to use to generate spreadsheet files compatible with Microsoft Excel versions 95 to 2003.
...
sentry: Sentry's Python SDK enables automatic reporting of errors and exceptions as well as identifies performance issues in your application.
...
flask_restful: 
random : Python Random module is an in-built module of Python which is used to generate random numbers. These are pseudo-random numbers means these are not truly random.
...
requests: Requests is a Python Library that lets you send HTTP/1.1 requests,
...
5. Serialization Deserialization:
	...
	- The json. dump() method (without “s” in “dump”) used to write Python serialized object as JSON formatted 
      data into a file.
...
-------------------------------------------------------------------------------------------------------------------------------------
1. Why we need to do Exception Handling
...
2. Exception class hierarchy
	...
	-The Python exception class hierarchy consists of a multiple exception classes spread across a 
     handful of important base class types. ... 
...
3. try except else finally. Explain in detail.
	...
	- try    : block lets you test a block of code for errors. If any code which may causes exception, 
              we can add the piece of code in try block 
...
4. how to handle mulitple exceptions
	- You can also handle multiple exceptions using a single except clause by passing these exceptions to the 
      clause as a tuple. Finally, you can also leave out the name of the exception after the except keyword
...
4. __enter__ vs __exit__ methods 
        __enter__ method is called at the start of with block before loading the file  
        __exit__    method is called at the end, once operations on file are completed 
	Note: with statement only works with objects that support the context management protocol 
...
1. Iterator protocol in python 
	- The iterator protocol defines a standard way to produce a sequence of values 
      ...
      So while working with for loop on sequence first iter method will be called on iterable object like 
      string list etc., and iterator object will be created.
...
3. Iterator vs generator
	...
	Iterator  : Iterator are objects which uses next() method to get next value of sequence.
 	Generator : A generator is a function that produces or yields a sequence of values using yield method.
...
1. What is decorator
    ...
    < ADD DETAILED NOTES HERE WITH ONE SIMPLE EXAMPLE >
